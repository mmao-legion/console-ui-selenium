...
//        }
    ...
    @Enterprise(name = "Vailqacn_Enterprise")
    @TestName(description = "Verify cannot edit budget on P2P parent level when enable display budget config And Weekly")
    @Test(dataProvider = "legionTeamCredentialsByRoles", dataProviderClass = CredentialDataProviderSource.class)
    public void verifyCannotEditBudgetOnP2PParentLevelWhenEnableDisplayBudgetConfigAndWeeklyAsInternalAdmin(String browser, String username, String password, String location) throws Exception {
        try {
            SimpleUtils.assertOnFail("Dashboard page not loaded successfully!", dashboardPage.isDashboardPageLoaded(), false);
            scheduleCommonPage.clickOnScheduleConsoleMenuItem();
            scheduleCommonPage.clickOnScheduleSubTab(ScheduleTestKendraScott2.SchedulePageSubTabText.Forecast.getValue());
        ...
        scheduleCommonPage.clickOnScheduleSubTab(ScheduleTestKendraScott2.SchedulePageSubTabText.Forecast.getValue());
            ...
            //Check edit budget button is not present
            SimpleUtils.assertOnFail("Edit button present on forecast smart card!",
                    !forecastPage.isLaborBudgetEditBtnLoaded(), false);

            //Get the budget value on forecast smart card
            String budgetValueOnForecastSmartCardForP2P = forecastPage.getLaborBudgetOnSummarySmartCard();
            ...
            //Check the budget hrs on schedule smart card same as on forecast smart card in P2P parent level
            String budgetValueOnScheduleSmartCardForP2P = smartCardPage.getBudgetValueFromScheduleBudgetSmartCard();
            SimpleUtils.assertOnFail("The budget value on forecast smart card is: " + budgetValueOnForecastSmartCardForP2P
                            + ". The budget value on schedule smart card is: " + budgetValueOnScheduleSmartCardForP2P,
                    budgetValueOnForecastSmartCardForP2P.equals(budgetValueOnScheduleSmartCardForP2P), false);

            //Go to overview page
            scheduleCommonPage.clickOnScheduleConsoleMenuItem();

            //Check the budget value at P2P same with the value on schedule page
            String budgetValueOnOverviewPageForP2P = scheduleOverviewPage.getCurrentWeekBudgetHoursAtP2PParentLocation(location, "Current Week");
            budgetValueOnOverviewPageForP2P = String.valueOf((int) Float.parseFloat(budgetValueOnOverviewPageForP2P));
            SimpleUtils.assertOnFail("The budget value on overview page is:" + budgetValueOnOverviewPageForP2P
                            + ". The budget value on schedule smart card is: " + budgetValueOnScheduleSmartCardForP2P,
                    budgetValueOnOverviewPageForP2P.equals(budgetValueOnScheduleSmartCardForP2P), false);
            ...
            SimpleUtils.assertOnFail("The budget label fail to load! ",
                    scheduleOverviewPage.isBudgetLabelShow(), false);
            //Check the budget value same with the value on schedule page
            List<WebElement> scheduleOverViewWeeksForP2P = scheduleOverviewPage.getOverviewScheduleWeeks();
            HashMap<String, Float> overviewDataForP2P = scheduleOverviewPage.getWeekHoursByWeekElement(scheduleOverViewWeeksForP2P.get(1));

            String budgetValueOnDashboardPageForP2P = String.valueOf((int) Float.parseFloat(overviewDataForP2P.get("guidanceHours").toString()));

            SimpleUtils.assertOnFail("The budget value on overview page is:" + budgetValueOnDashboardPageForP2P
                            + ". The budget value on schedule smart card is: " + budgetValueOnScheduleSmartCardForP2P,
                    budgetValueOnDashboardPageForP2P.equals(budgetValueOnScheduleSmartCardForP2P), false);
